...
def main():
    ...
    if args.model == 'swin_base_patch4_window7_224_in22k':
        ...
        model = create_model(args.model, num_classes=args.num_classes, pretrained=True, drop_path_rate=args.drop_path,
                             r=args.r, scale=args.scale, log_file=args.log_file)
    else:
        raise NotImplementedError
    ...
    for epoch in range(1, num_epochs + 1):
        top1_acc_eval = train_one_epoch(epoch, model, loader_train, loader_eval, optimizer, criterion, args, autocast=autocast, model_ema=model_ema, loss_scaler=loss_scaler, mixup_fn=mixup_fn)
...
def train_one_epoch(epoch, model, loader, loader_eval, optimizer, loss_fn, args, autocast, model_ema=None, loss_scaler=None, mixup_fn=None):
