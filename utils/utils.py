...
@torch.no_grad()
def save(log_dir, model, str):
    model.eval()
    trainable = {}
    for n, p in model.named_parameters():
        if 'small_' in n or 'adapter' in n or 'head' in n:
            trainable[n] = p.data
    torch.save(trainable, './%s/%s_save_model.pt' % ( log_dir,str))


def load(log_dir, model, str):
    st = torch.load('./%s/%s_save_model.pt' % ( log_dir,str))
